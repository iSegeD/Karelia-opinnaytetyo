...
const createPost = async (req, res) => {
  ...
  if (!file) {
    httpError("Please upload a post thumbnail", 422);
  }

  const buffer = await sharp(file.buffer).jpeg({ quality: 90 }).toBuffer();

  if (buffer.length > MAX_THUMBNAIL_SIZE) {
    httpError("Thumbnail too large after processing (max. 2MB)", 422);
  }

  const thumbnailFileName = `${uuidv4()}.${EXT}`;
  const gcsFile = thumbnailsBucket.file(thumbnailFileName);
...
const editPost = async (req, res) => {
  ...
  if (tags !== undefined) {
    updatedFields.tags = tags ? tags.split(",").map((tag) => tag.trim()) : [];
  }
  ...
  if (file) {
    ...
    if (buffer.length > MAX_THUMBNAIL_SIZE) {
      httpError("Thumbnail is too large after processing (max. 2MB)", 422);
    }
    ...
    const newThumbnailFileName = `${uuidv4()}.${EXT}`;
    const gcsFile = thumbnailsBucket.file(newThumbnailFileName);
    await gcsFile.save(buffer, {
      contentType: "image/jpeg",
      resumable: false,
    });
...
const deletePost = async (req, res) => {
  ...
  try {
    ...
    if (error !== 404) {
      httpError("Failed to delete thumbnail", 404);
    }
  }
...
const deleteComment = async (req, res) => {
  ...
  const populatedPost = await Post.findByIdAndUpdate(id, {
